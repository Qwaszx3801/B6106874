...
func (uq *UserQuery) QueryUserID() *ScholarshipQuery {
	query := &ScholarshipQuery{config: uq.config}
...
func (uq *UserQuery) WithUserID(opts ...func(*ScholarshipQuery)) *UserQuery {
	query := &ScholarshipQuery{config: uq.config}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withUserID; query != nil {
		...
		for _, n := range neighbors {
			fk := n.user_user_id
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_user_id" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_user_id" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.UserID = append(node.Edges.UserID, n)
		}
	}
